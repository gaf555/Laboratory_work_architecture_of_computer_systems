...
НЕОБХОДИМО внести правки для задачи № 3
...
| Команда              | Действие                                                                     |
| -------------------- | ---------------------------------------------------------------------------- |
| nop; nop srm         | Ничего не делает                                                             |
| mov src, dest        | Присваивание dest = srs                                                      |
| movabs imm64, dreg64 | В 64-битном режиме присваивание абсолютного 64-битного адреса dreg64 = imm64 |
| lea smem, dreg       | Вычисление адреса smem и запись его в dreg, dreg = &smem                     |
...
| jCC src  | Условный переход по адресу src (если верное условие CC)            |

#### Вызов и возврат функций

| Команда   | Действие |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| call src  | Вызов подпрограммы - помещает в стек адрес следующей инструкции (адрес возврата) и переходит по адресу src                                             |
| ret [imm] | Возврат из подпрограммы - снимает со стека адрес возврата и помещает его в указатель команд. Если указан параметр imm, снимает со стека езё imm байтов |
